...
class CustomAgentTokenBufferMemory(BaseChatMemory):
    ...
    def save_context(self, inputs: Dict[str, Any], outputs: Dict[str, Any]) -> None:
        ...
        self.chat_memory.add_user_message(extract_prompt(input_str))
        used_tools = "Used following tools:\n"
        steps = _format_intermediate_steps(outputs[self.intermediate_steps_key])
        for i, msg in enumerate(steps):
            used_tools += "Tools and input: " + msg.content.strip() + "\n"
            used_tools += "Return value: " + outputs[self.intermediate_steps_key][i][1].strip() + "\n"
        self.chat_memory.add_ai_message(used_tools + output_str)
...
class CodeInterpreterSession:
    ...
    def _tools(self, additional_tools: list[BaseTool]) -> list[BaseTool]:
        return additional_tools + [
            StructuredTool(
                ...
                "Do NOT use \"\"\"(triple quote), but use \"\"(double quote) to descript string !"
                "This code might span several lines, but note that you should utilize proper indentation and line breaks to maintain readability and functionality."
                "Avoid using the ; character to divide lines. Remember, variables are retained across runs." 
                "Be sure to structure your code accurately and cleanly."
                "Variables are NOT preserved between runs. ",
    ...
    def _agent_executor(self) -> AgentExecutor:
        return AgentExecutor(
            ...
            memory=CustomAgentTokenBufferMemory(
                memory_key="chat_history", llm=self.llm, max_token_limit=4000)
        )
    ...
    def generate_response_sync(
        ...
        try:
            ...
            response = self.agent_executor(user_request.content)
