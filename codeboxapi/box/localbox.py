...
class LocalBox(BaseBox):
    ...
    def start(self) -> CodeBoxStatus:
        ...
        while True:
            try:
                response = requests.get(self.kernel_url, timeout=27000)
        ...
        # try:
        #     self.heartbeat_loop = asyncio.get_event_loop()
        # except RuntimeError:
        #     self.heartbeat_loop = asyncio.new_event_loop()
        #     asyncio.set_event_loop(self.heartbeat_loop)

        # self.heartbeat_loop.create_task(self.send_heartbeat())
    ...
    def _check_port(self) -> None:
        try:
            response = requests.get(f"http://localhost:{self.port}", timeout=27000)
        except requests.exceptions.ConnectionError:
            pass
        else:
            if response.status_code == 200:
    ...
    def status(self) -> CodeBoxStatus:
        ...
        return CodeBoxStatus(
            status="running"
            if self.kernel_id
            and requests.get(self.kernel_url, timeout=27000).status_code == 200
            else "stopped"
        )
